...
Antigravity keeps all of your state in the `data/` directory — the SQLite database
(`app.db`), the Fernet encryption key (`data/.app_key`), the vault, memory, RAG
indexes, personal documents, and uploads. The `scripts/antigravity-backup` tool
...
```bash
# Create a snapshot → backups/antigravity-backup-<YYYYMMDD-HHMMSS>.tar.gz
./scripts/antigravity-backup snapshot

# List existing snapshots (most recent first)
./scripts/antigravity-backup list

# Check a tarball's integrity without extracting it
./scripts/antigravity-backup verify backups/antigravity-backup-20260101-120000.tar.gz

# Restore (destructive — see the warning below)
./scripts/antigravity-backup restore backups/antigravity-backup-20260101-120000.tar.gz --yes
```
...
./scripts/antigravity-backup snapshot --out /mnt/nas/antigravity-$(date +%F).tar.gz

# Full snapshot including research runs and mail attachments
./scripts/antigravity-backup snapshot --include-research --include-attachments
```
...
```cron
0 3 * * *  cd /path/to/antigravity && ./scripts/antigravity-backup snapshot --out "/mnt/nas/antigravity-$(date +\%F).tar.gz"
```
...
> **not** under `./data`. `antigravity-backup` therefore does not capture the Docker
