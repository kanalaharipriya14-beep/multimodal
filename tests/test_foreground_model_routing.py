...
def test_direct_low_signal_fallback_estimates_winning_route_prompt(
    ...
    backup = ("https://backup.example/v1", "antigravity-qwen-backup", {})
    candidate_requests = []
...
def test_skill_activation_reaches_later_fallback_request_and_pinned_round(monkeypatch):
    ...
    assert any(
        "route=antigravity-qwen-backup; tools=grep,manage_skills" in (message.get("content") or "")
