...
base compose with only the matching overlay merged into the ``antigravity``
...
def _merge_overlay_into_base(base: dict, overlay: dict) -> dict:
    """Build the expected standalone config: base + overlay on antigravity only."""
...
# --- Non-antigravity services and volumes untouched ---------------------------


@pytest.mark.parametrize("standalone_path", [NVIDIA_STANDALONE, AMD_STANDALONE])
def test_non_multivision_services_match_base(base, standalone_path):
...
# --- antigravity = base service + only the overlay additions ------------------


def test_nvidia_multivision_adds_only_overlay(base):
    standalone = _load(NVIDIA_STANDALONE)
...
def test_amd_multivision_adds_only_overlay(base):
    standalone = _load(AMD_STANDALONE)
    svc = standalone["services"][SERVICE]
    base_svc = base["services"][SERVICE]
...
def test_base_has_no_host_docker_access(base):
    ...
    assert "MULTIVISION_ENABLE_HOST_DOCKER=true" not in service["environment"]
...
def test_base_plus_host_docker_overlay_has_explicit_access(base):
    ...
    assert "MULTIVISION_ENABLE_HOST_DOCKER=true" in service["environment"]
