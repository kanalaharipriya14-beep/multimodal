...
in-process 30s poller and the `antigravity-mail poll-scheduled` CLI, which the
project's own docstrings warn can race on the same SQLite when
MULTIVISION_INPROCESS_POLLERS is left enabled alongside an external cron/systemd
