...
def test_build_provider_has_multivision_client_metadata():
    p = mcp_oauth.build_provider("srv-1", "https://example.com/mcp")
    md = p.context.client_metadata
    assert md.client_name == "Antigravity"
...
def test_redirect_base_defaults_to_the_bound_port(monkeypatch):
    ...
    # callback pinned to 7000 lands on AirPlay instead of Antigravity.
...
def test_redirect_base_prefers_the_explicit_origin(monkeypatch):
    ...
    resolved = _resolve_base(
        monkeypatch, OAUTH_REDIRECT_BASE_URL="https://antigravity.example/", APP_PORT="7860"
    )
    assert resolved == "https://antigravity.example"
...
def _authorize_page():
    ...
    return _oauth_authorize_page(
        ...
        "srv-1",
        "https://antigravity.example.com/api/mcp/oauth/callback",
    )
...
def test_redirect_base_override_is_forwarded_into_the_container():
    ...
    for name in _COMPOSE_FILES:
        ...
        environment = set(compose["services"]["antigravity"]["environment"])
