...
def _run_scheduler_scenario(scenario: str):
    ...
    script = f"""
      ...
      const window = {{
        __multivisionChatBusy: false,
        __multivisionChatBusyUntil: 0,
...
def test_temporary_chat_priority_retries_one_single_flight_until_idle():
    out = _run_scheduler_scenario("""
      window.__multivisionChatBusyUntil = 10000;
      let taskCalls = 0;
...
def test_active_prewarm_is_aborted_and_retried_once_after_priority_transition(transition):
    block = (
        "window.__multivisionChatBusy = true; "
        "window.dispatchEvent({ type: 'antigravity:chat-busy-change' });"
        if transition == "busy"
        else "document.visibilityState = 'hidden'; document.dispatchEvent({ type: 'visibilitychange' });"
    )
    unblock = (
        "window.__multivisionChatBusy = false; window.__multivisionChatBusyUntil = now; "
        "window.dispatchEvent({ type: 'antigravity:chat-busy-change' });"
        if transition == "busy"
    ...
    out = _run_scheduler_scenario(f"""
      ...
      console.log(JSON.stringify({{
        ...
        timers: timers.size, idleCallbacks: idleCallbacks.size,
        chatListeners: window.listenerCount('antigravity:chat-busy-change'),
