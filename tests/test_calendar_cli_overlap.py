"""Regression: `antigravity-calendar list` must select events that OVERLAP the
...
`antigravity-calendar list` with the default start=now()).
"""
...
def _load_cli(monkeypatch, rows):
    ...
    path = ROOT / "scripts" / "antigravity-calendar"
    loader = importlib.machinery.SourceFileLoader("multivision_calendar_cli", str(path))
