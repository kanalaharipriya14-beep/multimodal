...
def test_login_page_uses_mount_aware_urls():
    ...
    assert "window.__multivisionLoginAppUrl" in html
    for api_path in (
        "/api/version",
        "/api/auth/policy",
        "/api/auth/status",
    ...
    assert (
        "__multivisionLoginAppUrl || ((path) => path))('/static/js/theme.js')"
        in html
    )
...
def test_login_page_static_assets_resolve_under_mount_path():
    ...
    mounted_login_url = "https://example.test/antigravity/login"
    for relative_asset, mounted_path in (
        ("static/manifest.json", "/antigravity/static/manifest.json"),
        ("static/icons/icon-192.png", "/antigravity/static/icons/icon-192.png"),
        (
            "static/fonts/FiraCode-Regular.woff2",
            "/antigravity/static/fonts/FiraCode-Regular.woff2",
        ),
        (
            "static/fonts/FiraCode-SemiBold.woff2",
            "/antigravity/static/fonts/FiraCode-SemiBold.woff2",
        ),
    ):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("/antigravity", "/antigravity/api/models", "/api/models"),
        ("/antigravity/", "/antigravity//api/models", "/api/models"),
        ("/", "//api/models", "/api/models"),
        ("/antigravity", "/odyssey/api/models", "/odyssey/api/models"),
...
def test_real_auth_middleware_uses_application_relative_path(tmp_path):
    ...
    env.update({
        ...
        "MULTIVISION_DATA_DIR": str(tmp_path),
        "MULTIVISION_DISABLE_MCP": "1",
        "OPENAI_API_KEY": "",
        "PYTHONPATH": str(ROOT),
    ...
    probe = textwrap.dedent(
        ...
        async def main():
            setup = await _case("/antigravity", "/api/auth/setup", configured=False)
            mounted_api = await _case("/antigravity", "/api/models", configured=True)
            mounted_browser = await _case("/antigravity", "/notes", configured=True)
            webhook = await _case(
                "/antigravity",
