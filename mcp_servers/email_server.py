...
_MCP_OWNER_ARG = "_multivision_owner"
_CURRENT_OWNER: ContextVar[str | None] = ContextVar("email_mcp_owner", default=None)
_OWNER_ENV_KEYS = ("MULTIVISION_MCP_EMAIL_OWNER", "MULTIVISION_EMAIL_OWNER")
_OWNER_SCOPE_ERROR = (
    "Error: email MCP requires an authenticated owner or MULTIVISION_MCP_EMAIL_OWNER "
...
def _default_document_owner() -> str | None:
    ...
    """
    owner = os.environ.get("MULTIVISION_DOCUMENT_OWNER", "").strip()
    if owner:
        return owner
    try:
...
def _fixture_account_rows() -> list[dict]:
    ...
    for row in _fixture_email_rows(owner or None):
        email_addr = row.get("account_email") or owner or "fixture@fixtures.antigravity.local"
        if email_addr not in owners:
            owners.append(email_addr)
    if not owners:
        owners = [owner or "fixture@fixtures.antigravity.local"]
    return [
        {
            "id": "fixture-email",
    ...
    try:
        ...
        conn.execute("""
            CREATE TABLE IF NOT EXISTS scheduled_emails (
                ...
                owner TEXT DEFAULT '',
                account_id TEXT,
                multivision_kind TEXT
            )
        """)
        conn.execute("""
            INSERT INTO scheduled_emails
            (id, to_addr, cc, bcc, subject, body, in_reply_to, references_hdr,
             attachments, send_at, created_at, status, account_id, multivision_kind, owner)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'agent_draft', ?, ?, ?)
        """, (
            pending_id,
...
def _create_email_draft_document(
    *,
    ...
    """Create an Antigravity email compose document for user review. Does not send."""
...
def _draft_reply_to_email(uid, body, folder="INBOX", reply_all=False, account=None, title=None):
    """Create a threaded Antigravity reply draft document. Does not send."""
    conn = _imap_connect(account)
    conn.select(_q(folder), readonly=True)
    status, msg_data = conn.uid("FETCH", _b(uid), "(BODY.PEEK[])")
    conn.logout()
...
async def _ai_draft_reply_to_email(uid, folder="INBOX", reply_all=False, account=None, title=None):
    """Generate a reply with Antigravity' AI-reply prompt/style, then create a compose doc."""
...
async def list_tools() -> list[Tool]:
    ...
    return [
        Tool(
            ...
            description=(
                "List the email accounts configured in Antigravity. Returns each account's "
        ...
        Tool(
            ...
            description=(
                ...
                "draft_email so the user can review and send from Antigravity. "
        ...
        Tool(
            name="draft_email",
            description=(
                "Create a new Antigravity email compose draft document. This DOES NOT send. "
                ...
                "can edit or press Send in Antigravity. "
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "cc": {"type": "string", "description": "CC address(es), comma-separated (optional)"},
                    "bcc": {"type": "string", "description": "BCC address(es), comma-separated (optional)"},
                    "title": {"type": "string", "description": "Optional Antigravity document title"},
                    **ACCOUNT_PROP,
                },
                "required": ["to", "subject", "body"],
            },
        ...
        Tool(
            ...
            description=(
                ...
                "draft_email_reply so the user can review and send from Antigravity. "
        ...
        Tool(
            ...
            description=(
                "Create an Antigravity email reply draft document for an existing email UID. "
        ...
        Tool(
            ...
            description=(
                "Generate an AI reply using Antigravity' existing AI Reply behavior, "
    ...
    try:
        ...
        if name == "list_emails":
            ...
            if result.get("pending"):
                return [TextContent(
                    ...
                    text=(
                        ...
                        "Nothing has been sent yet. Review and approve it in Antigravity before delivery."
                    ),
                )]
            ...
            return [TextContent(
                type="text",
                text=(
                    f"Created Antigravity email draft `{result['title']}` "
                    f"(document ID: {result['doc_id']}){acct_note}. "
                    "It has not been sent; open the document in Antigravity to review and send."
                ),
            )]
            ...
            return [TextContent(
                type="text",
                text=(
                    f"Created Antigravity reply draft `{result['title']}` for UID {uid} "
                    f"(document ID: {result['doc_id']}){acct_note}. "
                    "It has not been sent; open the document in Antigravity to review and send."
                ),
            )]
            ...
            return [TextContent(
                type="text",
                text=(
                    f"Generated AI reply and created Antigravity compose draft "
