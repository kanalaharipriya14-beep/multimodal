...
async def _generate_pollinations_image(prompt: str, size: str = "1024x1024") -> list[TextContent]:
    import urllib.parse
    import httpx
    from pathlib import Path
    import uuid
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            if resp.status_code == 200 and len(resp.content) > 1000:
                ...
                result = (
                    ...
                    f"model: pollinations-flux\nsize: {size}"
                )
                return [TextContent(type="text", text=result)]
            return [TextContent(type="text", text=f"Error: Pollinations image generation failed ({resp.status_code})")]
...
async def call_tool(name: str, arguments: dict) -> list[TextContent]:
    ...
    try:
        ...
        # Auto-detect best available image model or fall back to Pollinations AI
        if not model_spec:
            for candidate in ("gpt-image-1.5", "gpt-image-1", "dall-e-3"):
                try:
                    ...
                    continue
            if not model_spec:
                return await _generate_pollinations_image(prompt, size)
