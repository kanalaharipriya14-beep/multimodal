...
def _local_model_gate_enabled() -> bool:
    return os.getenv("MULTIVISION_LOCAL_MODEL_GATE", "true").lower() not in {"0", "false", "no", "off"}
...
def _ollama_normalize_messages(messages: List[Dict]) -> List[Dict]:
    """Adapt Antigravity' canonical OpenAI-style messages to native Ollama /api/chat.

    Two shape mismatches silently break requests:

    1. Tool calls: Antigravity carries `function.arguments` as a JSON *string*.
    ...
    2. Images (issue #4723): Antigravity carries multimodal user content as an
...
def _apply_local_cache_affinity(payload: Dict, url: str, session_id: Optional[str]) -> None:
    ...
    Sending a stable ``session_id`` (derived from the Antigravity session) lets
...
def _provider_headers(provider: str, headers: Optional[Dict] = None) -> Dict[str, str]:
    ...
    if provider == "openrouter":
        h.setdefault("HTTP-Referer", "https://github.com/antigravity-dev/antigravity")
        h.setdefault("X-OpenRouter-Title", "Antigravity")
    if provider == "copilot":
...
def _format_upstream_error(status: int, body: bytes | str, url: str) -> str:
    ...
    if status == 429:
        if provider == "Google" or "generativelanguage" in url:
            return "Gemini is temporarily rate-limited. Please wait a moment and try again."
        return f"{provider} is temporarily rate-limited. Please wait a moment and try again."
    if status >= 500:
...
def _calculate_backoff_delay(attempt: int, headers: Optional[Dict | httpx.Headers] = None, base_delay: float = 1.5, max_delay: float = 30.0) -> float:
    """Calculate exponential backoff delay with jitter, respecting Retry-After header if present."""
    import random
    if headers:
        try:
            retry_after = None
            ...
            if retry_after:
                ...
                if val > 0:
                    return min(max_delay, val + random.uniform(0.1, 0.5))
...
# value returns HTTP 400. Antigravity does not currently send the `thinking` mode
...
# MULTIVISION_MISTRAL_REASONING_EFFORT (e.g. set to "medium" for cheaper chat).
_MISTRAL_REASONING_EFFORT = os.getenv("MULTIVISION_MISTRAL_REASONING_EFFORT", "high")
...
def _sanitize_llm_messages(messages: List[Dict]) -> List[Dict]:
    """Strip Antigravity-only metadata before sending messages to providers.
...
async def llm_call_async(
    ...
    attempt = 0
    logger.info(f"LLM async request start: target={target_url}, model={model}, workload={workload}")
        ...
        try:
            ...
            if not r.is_success:
                ...
                if r.status_code in (429, 502, 503, 504) and attempt < max_retries:
                    retry_delay = _calculate_backoff_delay(attempt, r.headers)
                    logger.warning(
                        f"LLM async call rate-limited/failed: target={target_url}, model={model}, status={r.status_code}, "
                        f"attempt={attempt}/{max_retries}, retry_delay={retry_delay:.2f}s"
                    )
                    await asyncio.sleep(retry_delay)
                    continue
                logger.warning(
                    f"LLM async call failed: target={target_url}, model={model}, duration={duration:.2f}s, "
                    f"status={r.status_code}, detail={friendly}"
                )
                raise HTTPException(r.status_code, friendly)
            logger.info(
                f"LLM async request completed: target={target_url}, model={model}, status={r.status_code}, "
                f"duration={duration:.2f}s, retries={attempt - 1}"
            )
...
async def _stream_llm_inner(url: str, model: str, messages: List[Dict], temperature: float = LLMConfig.DEFAULT_TEMPERATURE,
    ...
    if provider == "anthropic":
        ...
        # Effort level is configurable via MULTIVISION_MISTRAL_REASONING_EFFORT
    ...
    try:
        ...
        logger.info(f"LLM stream request start: target={target_url}, model={model}")
        while stream_attempt < max_stream_retries:
            ...
            async with client.stream('POST', target_url, json=payload, headers=h, timeout=stream_timeout) as r:
                ...
                if r.status_code != 200:
                    raw = (await r.aread()).decode(errors="replace")
                    friendly = _format_upstream_error(r.status_code, raw, target_url)
                    if r.status_code in (429, 502, 503, 504) and stream_attempt < max_stream_retries:
                        retry_delay = _calculate_backoff_delay(stream_attempt, r.headers)
                        logger.warning(
                            f"LLM stream connect rate-limited/failed: target={target_url}, model={model}, status={r.status_code}, "
                            f"attempt={stream_attempt}/{max_stream_retries}, retry_delay={retry_delay:.2f}s"
                        )
                        await asyncio.sleep(retry_delay)
                        continue
                    logger.warning(
                        f"LLM stream connect failed: target={target_url}, model={model}, status={r.status_code}, text={friendly}"
                    )
                    yield f'event: error\ndata: {json.dumps({"status": r.status_code, "text": friendly, "raw": raw[:500]})}\n\n'
                    return

                logger.info(
                    f"LLM stream request completed handshake: target={target_url}, model={model}, status={r.status_code}, "
                    f"retries={stream_attempt - 1}"
                )
