...
class TaskScheduler:
        ...
        try:
            ...
            if gate_foreground:
                ...
                if waiting and waiting.status == "queued":
                    waiting.result = "Queued — waiting for Antigravity to be idle…"
                    db.commit()
                ...
                async def _cancel_if_foreground_active():
                    ...
                    while True:
                        ...
                        if has_foreground_activity():
                            ...
                            logger.info("Task '%s' interrupted because Antigravity became active", task.name)
                            if current_task:
                                current_task.cancel()
                            return
            ...
            try:
                ...
                msg = (
                    "Paused because Antigravity became active"
    ...
    async def _deliver_via_email(self, output: str, task, result: str):
        ...
        try:
            ...
            msg["X-Antigravity-Origin"] = "antigravity-ui"
            msg["X-Antigravity-Kind"] = "task"
            msg["X-Antigravity-Ref"] = str(task.id)
    ...
    async def _deliver_via_mcp(self, tool_name: str, task, result: str):
        ...
        args = {
            ...
            "body": result,
            "headers": {
                "X-Antigravity-Origin": "antigravity-ui",
                "X-Antigravity-Kind": "task",
                "X-Antigravity-Ref": str(task.id),
            },
        }
        if recipient:
    ...
    async def stop_background_tasks_for_foreground(self, *, reason: str = "Antigravity became active") -> int:
        ...
        user opens or uses Antigravity, foreground interaction wins immediately.
