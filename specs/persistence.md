...
- `routes/backup_routes.py` and `scripts/antigravity-backup`;
...
Antigravity does not use Alembic. `core.database.init_db()` runs at module import, before FastAPI lifespan startup. `Base.metadata.create_all()` creates missing tables; hand-written `_migrate_*` functions add or reshape legacy columns.
