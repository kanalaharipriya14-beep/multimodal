...
- CLI helper `scripts/antigravity-mcp`;
...
caller or configured through `MULTIVISION_MCP_EMAIL_OWNER` /
`MULTIVISION_EMAIL_OWNER`; if owner-scoped email accounts exist and no owner is
...
- Docker images include selected shell dependencies and the Docker CLI, but host Docker socket access from inside the app container remains unavailable unless the operator explicitly enables `docker/host-docker.yml`/`MULTIVISION_ENABLE_HOST_DOCKER=true` and mounts a real socket.
