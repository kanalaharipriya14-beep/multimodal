                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in Antigravity (if public URL is configured).
                                    ...
                                    alert_body = (
                                        ...
                                        f"Reason: {reason}\n\n"
                                        + (f"Open in Antigravity: {open_url}\n\n" if open_url else "")
                                        + f"---\n"
                                        f"From: {sender}\n"
                                        f"Subject: {subject}\n"
                                        f"Date: {msg.get('Date','')}\n\n"
                                    ...
                                    # HTML alternative with a clickable "Open in Antigravity" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "multivision_kind" if "multivision_kind" in cols else "'scheduled' AS multivision_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                # externally cron-driven `antigravity-mail poll-scheduled`, or
                # an admin running the CLI manually alongside the in-process
                # one despite the MULTIVISION_INPROCESS_POLLERS=0 guidance) -
                ...
                multivision_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-Antigravity-Origin"] = "antigravity-ui"
                outer["X-Antigravity-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", multivision_kind or "scheduled")[:64]
                outer["X-Antigravity-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `antigravity-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `MULTIVISION_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `antigravity-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("MULTIVISION_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `MULTIVISION_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (MULTIVISION_INPROCESS_POLLERS=0); "
            "drive `antigravity-mail poll-scheduled` externally."
        )
        return
    import asyncio
