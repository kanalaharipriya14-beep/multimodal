...
let _sortMode = Storage.get('antigravity-session-sort') || 'active'; // default to last active
...
function _shouldPreserveStartupComposer(msgInput) {
  ...
  if (window.__multivisionComposerUserEdited) return true;
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'Antigravity Chat';
...
const FOLDER_STATE_KEY = 'antigravity-folder-state';
const FOLDER_ORDER_KEY = 'antigravity-folder-order';
...
export async function selectSession(id, { keepSidebar = false, showLoading = true, immediateLoading = false } = {}) {
  ...
  try {
    ...
    currentSessionId = id;
    try { window.__multivisionLastSelectedSessionId = id; } catch (_) {}
    ...
    // URL hash — the user complained that coming back to Antigravity kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'Antigravity Chat';
    }
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('antigravity-doc-open-' + id) === '1';
...
export function createDirectChat(url, modelId, endpointId, opts = {}) {
  ...
  currentSessionId = null;
  try { window.__multivisionLastSelectedSessionId = ''; } catch (_) {}
  Storage.remove('lastSessionId');
  history.replaceState(null, '', window.location.pathname);
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('antigravity-session-sort', mode);
  else Storage.remove('antigravity-session-sort');
  renderSessionList();
}
