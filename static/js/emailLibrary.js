    ...
    <div class="email-settings-section email-settings-style-section">
      <div class="email-settings-section-head">
        <div>
          ...
          <div class="email-settings-section-desc">Used when Antigravity drafts replies for you.</div>
        </div>
      </div>
      <label class="email-settings-field">
...
document.addEventListener('antigravity:email-filter-tag', (e) => {
...
try { window.__multivisionGetActiveEmailContext = _getActiveEmailContext; } catch (_) {}
...
window.addEventListener('antigravity-reminder-channel-changed', (e) => {
...
const _LIB_SESSION_CACHE_PREFIX = 'antigravity.email.list.';
const _LIB_SESSION_CACHE_TTL_MS = 10 * 60 * 1000;
const _LIB_LAST_ACCOUNT_KEY = 'antigravity.email.lastAccountId';
...
function _isChatInteractionBusy() {
  try {
    if (window.__multivisionChatBusy) return true;
    const until = Number(window.__multivisionChatBusyUntil || 0);
...
function _scheduleEmailPrewarm(task, { delay = 0 } = {}) {
  ...
  window.addEventListener('antigravity:chat-busy-change', handlePriorityChange);
  document.addEventListener('visibilitychange', handlePriorityChange);
  _libPrewarmDetachPriorityListeners = () => {
    window.removeEventListener('antigravity:chat-busy-change', handlePriorityChange);
...
function _publishActiveAccount() {
  try { window.__multivisionActiveEmailAccount = state._libAccountId || null; } catch (_) {}
  try {
...
export function openEmailLibrary(opts = {}) {
        ...
        <div class="admin-card" style="flex:1;flex-direction:column;display:flex;overflow:hidden;">
          ...
          <div class="memory-toolbar">
            <div class="memory-category-filters">
              ...
              <button class="memory-toolbar-btn email-reminders-clear-btn hidden" id="email-reminders-clear-btn" title="Permanently delete Antigravity reminder emails">
            ...
            <div class="email-search-wrap" style="position:relative;flex:1;min-width:140px;">
              ...
              <button class="memory-toolbar-btn email-reminder-toggle-inline hidden" id="email-reminder-btn" title="Show Antigravity reminder emails">
  ...
  if (tagsToggle) {
    ...
    tagsToggle.addEventListener('click', () => {
      ...
      localStorage.setItem('antigravity.email.showTags', state._libShowTags ? '1' : '0');
      tagsToggle.classList.toggle('active', !!state._libShowTags);
      tagsToggle.setAttribute('aria-pressed', String(!!state._libShowTags));
      _renderGrid();
      document.dispatchEvent(new CustomEvent('antigravity:email-tags-toggle', { detail: { show: state._libShowTags } }));
    });
  }
  document.getElementById('email-reminders-clear-btn')?.addEventListener('click', async () => {
    const ok = await styledConfirm('Permanently delete all Antigravity reminder emails?', {
      confirmText: 'Delete',
      cancelText: 'Cancel',
      danger: true,
    });
    if (!ok) return;
    try {
      const res = await fetch(`${API_BASE}/api/email/antigravity/reminders?permanent=1${_acct()}`, {
        method: 'DELETE',
        credentials: 'same-origin',
      });
      const data = await res.json().catch(() => ({}));
...
const _BUBBLES_DISABLED_KEY = 'antigravity.email.bubblesDisabled';
...
const _SUMMARY_COLLAPSED_KEY = 'antigravity.email.summaryCollapsed';
