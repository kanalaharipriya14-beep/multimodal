...
document.addEventListener('antigravity:tool-approval', () => {
...
export function handleUIControl(uiData) {
  ...
  try {
    if (uiEvent === 'toggle' || uiData.ui_event === 'toggle') {
      ...
      document.querySelectorAll('.antigravity-highlight').forEach(function(e) { e.classList.remove('antigravity-highlight'); });
      document.querySelectorAll('.antigravity-hl-label').forEach(function(e) { e.remove(); });
      var target = document.querySelector(uiData.selector);
      if (target) {
        target.classList.add('antigravity-highlight');
        target.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
        if (uiData.label) {
          var lbl = document.createElement('div');
          lbl.className = 'antigravity-hl-label';
