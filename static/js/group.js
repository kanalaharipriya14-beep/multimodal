...
const GROUP_STATE_KEY = 'antigravity-group-state';
...
export async function startGroup(models, parentSessionId) {
  ...
  try {
    ...
    try {
      const storedGroupSessions = Storage.getJSON('antigravity-group-sessions', []);
      const gids = Array.isArray(storedGroupSessions) ? storedGroupSessions : [];
      if (!gids.includes(_parentSessionId)) { gids.push(_parentSessionId); localStorage.setItem('antigravity-group-sessions', JSON.stringify(gids)); }
    } catch (e) {}
  } catch (e) {
