...
const MODEL_INFO = {
  ...
  // --- Google ---
  'gemini-3.6-pro':       { input: 1.25,  output: 10.00, ctx: 1048576 },
  'gemini-3.6-flash':     { input: 0.15,  output: 0.60,  ctx: 1048576 },
...
const _COST_LEDGER_LOCK = 'antigravity-session-cost-ledger';
...
const _ACTION_RECENTS_KEY = 'antigravity-msg-actions-recent';
 ...
 */
const _USER_ACTION_RECENTS_KEY = 'antigravity-user-actions-recent';
    ...
    ctxRing.addEventListener('click', (e) => {
      ...
      if (compactBtn) {
        compactBtn.addEventListener('click', async (e) => {
          ...
          compactRole.textContent = 'Antigravity';
      ...
      row.addEventListener('click', () => {
        if (isToolApproval) {
          ...
          if (onSubmit) {
            ...
            card.remove();
            document.dispatchEvent(new CustomEvent('antigravity:tool-approval', { detail }));
          }
        } else {
          send(label);
        }
...
export function addMessage(role, content, modelName, metadata) {
  try {
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'Antigravity' : modelRouteLabel(
