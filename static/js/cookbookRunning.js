...
function _buildCrashReport(task, outputText) {
  ...
  const report = [
    '## Antigravity Cookbook crash report',
    '',
...
function _winSessionCmd(task, tmuxArgs) {
  const host = _taskRemoteHost(task);
  const sd = host ? '$env:TEMP\\antigravity-sessions' : '$env:TEMP\\antigravity-tmux';
  ...
  if (tmuxArgs.includes('capture-pane')) {
    ...
    const ps = host
      ...
      : `Get-Content (Join-Path $env:TEMP 'antigravity-tmux\\${sid}.log') -Tail ${lines} -ErrorAction SilentlyContinue`;
  ...
  if (tmuxArgs.includes('has-session')) {
    const ps = host
      ...
      : `$p = Get-Content (Join-Path $env:TEMP 'antigravity-tmux\\${sid}.pid') -ErrorAction SilentlyContinue; if ($p) { Get-Process -Id $p -ErrorAction SilentlyContinue | Out-Null; if ($?) { exit 0 } else { exit 1 } } else { exit 1 }`;
    return _winPowerShellCmd(task, ps);
  }
  ...
  if (tmuxArgs.includes('send-keys') && tmuxArgs.includes('C-c')) {
    const ps = host
      ...
      : `$p = Get-Content (Join-Path $env:TEMP 'antigravity-tmux\\${sid}.pid') -ErrorAction SilentlyContinue; if ($p) { Stop-Process -Id $p -ErrorAction SilentlyContinue }`;
    return _winPowerShellCmd(task, ps);
  }
...
function _winSessionStopTreePs(task) {
  ...
  return host
    ...
    : `${stopTree}; $p = Get-Content (Join-Path $env:TEMP 'antigravity-tmux\\${sid}.pid') -ErrorAction SilentlyContinue; if ($p -match '^\\d+$') { Stop-Tree ([int]$p) }; Remove-Item (Join-Path $env:TEMP 'antigravity-tmux\\${sid}.*') -Force -ErrorAction SilentlyContinue`;
}
  ...
  if (task.payload) {
    if (task.type === 'serve' && task.payload._cmd) {
      ...
      _updateTask(task.sessionId, {
        status: 'running',
        output: `${task.output || ''}\n\n[antigravity] Retrying download. Progress may briefly look like a fresh download while HuggingFace checks cached/incomplete files; cached partial files will be reused when available.`.trim(),
        _retrying: true,
      });
      ...
      const _openTaskMenu = (e) => {
        ...
        if (_isWindows(task)) {
          ...
          const logCmd = host
            ...
            : `powershell -Command "Get-Content (Join-Path $env:TEMP 'antigravity-tmux\\${task.sessionId}.log') -Wait"`;
...
const BG_LEADER_KEY = 'antigravity-cookbook-bg-leader';
...
function _foregroundChatBusy() {
  try {
    return !!window.__multivisionChatBusy || Date.now() < (window.__multivisionChatBusyUntil || 0);
  } catch {
    return false;
  }
}
