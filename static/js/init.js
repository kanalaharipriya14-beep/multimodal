// Antigravity UI — Initialization Scripts
...
function markComposerUserEdited() {
  ...
  msgInput.addEventListener('input', () => {
    window.__multivisionComposerUserEdited = !!msgInput.value;
  });
}
...
function clearFreshComposerRestore() {
  ...
  if (window.__multivisionComposerUserEdited || document.activeElement === msgInput) return;
  if (msgInput.value) {
    msgInput.value = '';
...
(async () => {
  try {
    ...
    if (!liveUser) return;
    const KEY = 'antigravity-auth-user';
    const cachedUser = localStorage.getItem(KEY);
    if (cachedUser && cachedUser !== liveUser) {
      const _keepKeys = new Set(['antigravity-last-user', KEY]);
      const toRemove = [];
