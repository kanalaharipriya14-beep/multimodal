...
import { bindMenuDismiss, dismissOrRemove } from './escMenuStack.js';
  ...
  const _EMAIL_LOCAL_DRAFT_PREFIX = 'antigravity.email.replyDraft.v1:';
  ...
  async function _resolveComposeSendAccountId() {
    const activeAccountId = window.__multivisionActiveEmailAccount || null;
  ...
  const _docOpenKey = (sessionId) => 'antigravity-doc-open-' + sessionId;
  const _docMinimizedKey = (sessionId) => 'antigravity-doc-minimized-' + sessionId;
  ...
  let _multivisionAttachMenu = null;

  function _closeMultivisionAttachMenu() {
    if (_multivisionAttachMenu) {
      _multivisionAttachMenu.remove();
      _multivisionAttachMenu = null;
    }
  ...
  function _attachMenuOutsideClick(e) {
    if (_multivisionAttachMenu && !_multivisionAttachMenu.contains(e.target)) _closeMultivisionAttachMenu();
  }

  function _attachMenuEscape(e) {
    if (e.key !== 'Escape') return;
    _closeMultivisionAttachMenu();
  }

  function _positionMultivisionAttachMenu(anchor, menu) {
  ...
  function _multivisionAttachLabel(item, kind) {
    if (kind === 'gallery') {
  ...
  async function _stageMultivisionAttachment(kind, id) {
    const doc = docs.get(activeDocId);
    if (!doc || doc.language !== 'email') return null;
    if (!doc._composeAtts) doc._composeAtts = [];
    const res = await fetch(`${API_BASE}/api/email/compose-from-antigravity`, {
      method: 'POST',
      credentials: 'same-origin',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function _stageMultivisionZip(items) {
    const doc = docs.get(activeDocId);
    if (!doc || doc.language !== 'email') return null;
    if (!doc._composeAtts) doc._composeAtts = [];
    const res = await fetch(`${API_BASE}/api/email/compose-from-antigravity-zip`, {
      method: 'POST',
      credentials: 'same-origin',
      headers: { 'Content-Type': 'application/json' },
  ...
  function _afterMultivisionAttachmentsAdded(count, label) {
    _renderComposeAttachments();
    clearTimeout(_autoSaveDebounce);
    _autoSaveDebounce = setTimeout(() => { saveDocument({ silent: true }); }, 800);
  ...
  async function _attachMultivisionItem(kind, id, label, opts = {}) {
    try {
      const data = await _stageMultivisionAttachment(kind, id);
      if (!data) return;
      _afterMultivisionAttachmentsAdded(1, label || data.filename);
      if (!opts.keepOpen) _closeMultivisionAttachMenu();
    } catch (err) {
      console.error('Failed to attach Antigravity item:', err);
      if (uiModule) uiModule.showError('Failed to attach from Antigravity');
    }
  }

  function _selectedMultivisionAttachRows(menu) {
    return Array.from(menu?.querySelectorAll?.('.email-antigravity-attach-row.is-selected') || []);
  }

  function _syncMultivisionAttachSelection(menu) {
    const selected = _selectedMultivisionAttachRows(menu);
    const bar = menu?.querySelector?.('.email-antigravity-attach-actions');
    const count = menu?.querySelector?.('.email-antigravity-attach-count');
    const attachBtn = menu?.querySelector?.('.email-antigravity-attach-selected');
  ...
  async function _attachSelectedMultivisionItems(menu) {
    const rows = _selectedMultivisionAttachRows(menu);
    if (!rows.length) return;
    const btn = menu.querySelector('.email-antigravity-attach-selected');
    if (btn) {
      btn.disabled = true;
    ...
    try {
      ...
      if (zip) {
        await _stageMultivisionZip(items);
        added = 1;
      } else {
        for (const item of items) {
          await _stageMultivisionAttachment(item.kind, item.id);
          added += 1;
        }
      }
      _afterMultivisionAttachmentsAdded(added, zip ? 'antigravity-attachments.zip' : undefined);
      _closeMultivisionAttachMenu();
    } catch (err) {
      console.error('Failed to attach selected Antigravity items:', err);
      if (uiModule) uiModule.showError(added ? `Attached ${added}, then failed` : 'Failed to attach from Antigravity');
      _renderComposeAttachments();
    } finally {
      if (btn) {
  ...
  async function _loadMultivisionAttachItems(menu, kind) {
    const list = menu.querySelector('.email-antigravity-attach-list');
    if (!list) return;
    ...
    const q = (menu.querySelector('.email-antigravity-attach-search')?.value || '').trim();
    try {
      ...
      if (!items.length) {
        list.innerHTML = `<div class="email-antigravity-attach-empty">${q ? 'No matches' : `No ${kind === 'gallery' ? 'images' : 'documents'}`}</div>`;
        _syncMultivisionAttachSelection(menu);
        return;
      }
      list.innerHTML = '';
      for (const item of items) {
        const label = _multivisionAttachLabel(item, kind);
        const row = document.createElement('button');
        row.type = 'button';
        row.className = `email-antigravity-attach-row ${kind === 'gallery' ? 'is-gallery' : ''}`;
        row.dataset.id = item.id || '';
        row.dataset.kind = kind;
        if (kind === 'gallery') {
          const src = item.url ? `${API_BASE}${item.url}` : '';
          row.innerHTML = `
            <span class="email-antigravity-attach-dot" aria-hidden="true"></span>
            <span class="email-antigravity-attach-thumb">${src ? `<img src="${_escHtml(src)}" alt="">` : ''}</span>
            <span class="email-antigravity-attach-main">
              <span class="email-antigravity-attach-title">${_escHtml(label)}</span>
              <span class="email-antigravity-attach-meta">${_escHtml(item.filename || 'image')}</span>
            </span>
          `;
        } else {
          row.innerHTML = `
            <span class="email-antigravity-attach-dot" aria-hidden="true"></span>
            <span class="email-antigravity-attach-icon">${langIcon(item.language || 'text', 14, { style: 'opacity:0.8;' })}</span>
            <span class="email-antigravity-attach-main">
              <span class="email-antigravity-attach-title">${_escHtml(label)}</span>
              <span class="email-antigravity-attach-meta">${_escHtml(item.language || 'text')}</span>
            </span>
          `;
        }
        ...
        row.addEventListener('dblclick', () => _attachMultivisionItem(kind, item.id, label, { keepOpen: false }));
        list.appendChild(row);
      }
      _syncMultivisionAttachSelection(menu);
    } catch (err) {
      console.error('Failed to load Antigravity attach items:', err);
      list.innerHTML = '<div class="email-antigravity-attach-empty">Could not load</div>';
    }
  }
  ...
  function _showComposeAttachMenu(anchor) {
    ...
    menu.className = 'email-antigravity-attach-menu';
    menu.innerHTML = `
      <button type="button" class="email-antigravity-attach-local">
      ...
      <div class="email-antigravity-attach-tabs">
      ...
      <label class="email-antigravity-attach-search-wrap">
        ...
        <input type="search" class="email-antigravity-attach-search" placeholder="Search attachments">
      </label>
      <div class="email-antigravity-attach-list"></div>
      <div class="email-antigravity-attach-actions">
        <span class="email-antigravity-attach-count"></span>
        <button type="button" class="email-antigravity-attach-selected" disabled>
    ...
    _multivisionAttachMenu = menu;
    _positionMultivisionAttachMenu(anchor, menu);
    menu.querySelector('.email-antigravity-attach-local')?.addEventListener('click', () => {
      _closeMultivisionAttachMenu();
      document.getElementById('doc-email-file-input')?.click();
    });
    menu.querySelectorAll('[data-ody-attach-kind]').forEach(btn => {
      btn.addEventListener('click', () => _loadMultivisionAttachItems(menu, btn.dataset.odyAttachKind));
    });
    let attachSearchTimer = null;
    menu.querySelector('.email-antigravity-attach-search')?.addEventListener('input', () => {
      clearTimeout(attachSearchTimer);
      attachSearchTimer = setTimeout(() => {
        _loadMultivisionAttachItems(menu, menu.dataset.odyAttachKind || 'document');
      }, 220);
    });
    menu.querySelector('.email-antigravity-attach-selected')?.addEventListener('click', () => _attachSelectedMultivisionItems(menu));
    setTimeout(() => {
      document.addEventListener('click', _attachMenuOutsideClick, true);
      document.addEventListener('keydown', _attachMenuEscape, true);
    }, 0);
    _loadMultivisionAttachItems(menu, 'document');
  }
    ...
    try {
      const res = await fetch(`${API_BASE}/api/email/draft`, {
        ...
        body: JSON.stringify({
          ...
          account_id: window.__multivisionActiveEmailAccount || null,
        }),
      });
      const data = await res.json();
      if (data.success) {
  ...
  const _AI_REPLY_CONTEXT_STORE_PREFIX = 'antigravity:email-ai-reply-context:v1:';
  ...
  async function _aiReply(opts = {}) {
    ...
    const sourceUid = document.getElementById('doc-email-source-uid')?.value?.trim() || '';
    const sourceFolder = document.getElementById('doc-email-source-folder')?.value?.trim() || 'INBOX';
    const sourceAccountId = docs.get(activeDocId)?.sourceEmailAccountId || window.__multivisionActiveEmailAccount || '';
  ...
  export function openPanel() {
    ...
    pane.innerHTML = `
      ...
      <div id="doc-email-actions" class="doc-email-actions" style="display:none">
        ...
        <div class="email-send-split">
          <button type="button" id="doc-email-send-btn" class="email-send-btn email-send-main" title="Send email (Ctrl+Enter)" onpointerdown="window.multivisionEmailSendIntent&&window.multivisionEmailSendIntent(event)" onmousedown="window.multivisionEmailSendIntent&&window.multivisionEmailSendIntent(event)" onclick="window.multivisionEmailSendIntent&&window.multivisionEmailSendIntent(event)"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>Send</button>
          <button type="button" id="doc-email-send-caret" class="email-send-btn email-send-caret" title="More send options" aria-haspopup="true" aria-expanded="false" onpointerdown="window.multivisionEmailCaretIntent&&window.multivisionEmailCaretIntent(event)" onmousedown="window.multivisionEmailCaretIntent&&window.multivisionEmailCaretIntent(event)"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="6 9 12 15 18 9"/></svg></button>
    ...
    const handleSendIntent = (e) => {
      if (e && e.__multivisionEmailSendHandled) return;
      ...
      if (e) {
        e.preventDefault();
        e.stopPropagation();
        e.__multivisionEmailSendHandled = true;
      }
    ...
    window.multivisionEmailSendIntent = handleSendIntent;
    ...
    const handleCaretIntent = (e) => {
      if (e && e.__multivisionEmailCaretHandled) return;
      const now = Date.now();
      if (e && e.type === 'click' && now - lastCaretToggleAt < 350) {
        e.preventDefault();
        e.stopPropagation();
        e.__multivisionEmailCaretHandled = true;
        return;
      }
      const rawTarget = e && e.target;
      const target = rawTarget && rawTarget.nodeType === Node.TEXT_NODE ? rawTarget.parentElement : rawTarget;
    ...
    window.multivisionEmailCaretIntent = handleCaretIntent;
    ...
    let _fontIdx = parseInt(localStorage.getItem('antigravity-doc-fontsize') || '0', 10);
    ...
    function _applyDocFont() {
      ...
      localStorage.setItem('antigravity-doc-fontsize', _fontIdx);
    }
    _applyDocFont();
  ...
  const _DOC_RECENTS_KEY = 'antigravity-doc-actions-recent';
  ...
  function _saveSuggestionsToStorage() {
    ...
    if (data.length) {
      localStorage.setItem('antigravity-suggestions-' + activeDocId, JSON.stringify(data));
    } else {
      localStorage.removeItem('antigravity-suggestions-' + activeDocId);
    }
  }
  ...
  function _restoreSuggestionsFromStorage(docId) {
    try {
      const raw = localStorage.getItem('antigravity-suggestions-' + docId);
      if (!raw) return;
