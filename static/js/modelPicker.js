...
const RECENT_KEY = 'antigravity-model-recent';
const FAVORITES_KEY = 'antigravity-model-favorites';
const RECENT_MAX = 5;
...
async function _ensureDefaultPendingChat() {
  ...
  try {
    let dc = null;
    try {
      dc = window.__multivisionDefaultChat || null;
    } catch (_) {}
...
function _initModelPickerDropdown() {
  ...
  async function _refreshLocalProbe() {
    try {
      if (window.__multivisionChatBusy || Date.now() < (window.__multivisionChatBusyUntil || 0)) return;
    } catch (_) {}
    const now = Date.now();
  ...
  const _collapsedProviders = new Set(_loadList('antigravity-model-collapsed'));
    ...
    if (all.length <= BROWSE_ALL_LIMIT) {
      ...
      sorted.forEach(provider => {
        ...
        header.addEventListener('click', (e) => {
          ...
          _saveList('antigravity-model-collapsed', [..._collapsedProviders]);
          const st = listEl.scrollTop;
          _populate('');
          listEl.scrollTop = st;
        });
...
async function _pick(m) {
    _defaultPendingSeq++;
    try {
      window.__multivisionLastPickedRoute = {
        model: m.mid || '',
        endpoint_url: m.url || '',
    ...
    try { window.__multivisionModelSwitchPromise = switchPromise; } catch (_) {}
    const finishSwitch = () => {
      try {
        if (switchDone) switchDone();
        if (window.__multivisionModelSwitchPromise === switchPromise) delete window.__multivisionModelSwitchPromise;
      } catch (_) {}
    };
    ...
    try { document.dispatchEvent(new CustomEvent('antigravity:model-picked', { detail: m })); } catch {}
  ...
  document.addEventListener('antigravity:auto-select-model', async (e) => {
...
export function updateModelPicker() {
  ...
  if (!modelId && !currentSessionId && !_pendingChat && _deps.setPendingChat) {
    ...
    try {
      cachedDefault = window.__multivisionDefaultChat || null;
    } catch (_) {}
    if (!cachedDefault || !cachedDefault.endpoint_url || !cachedDefault.model) {
      try {
        cachedDefault = JSON.parse(localStorage.getItem('antigravity-default-chat-cache') || 'null');
      } catch (_) {}
    }
    if (cachedDefault && cachedDefault.endpoint_url && cachedDefault.model) {
  ...
  // SECURITY: deliberately NOT auto-injecting `antigravity-model-favorites[0]`
