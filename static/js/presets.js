  ...
  if (delBtn) {
    delBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (isBuiltin) {
          const hidden = loadStoredArray('antigravity-hidden-presets');
          if (!hidden.includes(charName)) hidden.push(charName);
          localStorage.setItem('antigravity-hidden-presets', JSON.stringify(hidden));
        }
...
function _populateCharSelect() {
  ...
  const hiddenPresets = loadStoredArray('antigravity-hidden-presets');
...
function initPersistentChat() {
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      const charSessions = loadStoredObject('antigravity-char-sessions');
      charSessions[sessionId] = charName;
      localStorage.setItem('antigravity-char-sessions', JSON.stringify(charSessions));
