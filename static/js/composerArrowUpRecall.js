...
export function wireArrowUpRecall(composer, getUserMessages, options = {}) {
  ...
  const debug = (...args) => {
    try {
      if (localStorage.getItem('multivisionArrowRecallDebug') === '1') {
  ...
  composer.addEventListener('input', () => {
    ...
    lastRecalledValue = '';
    recallHistory = [];
    try { delete composer.dataset.multivisionRecallIndex; } catch (_) {}
  });
  ...
  composer.addEventListener('keydown', (e) => {
    ...
    if (currentIndex < 0 && currentValue) {
      const markedIndex = Number(composer.dataset?.multivisionRecallIndex);
    ...
    if (e.key === 'ArrowDown') {
      ...
      try { composer.dataset.multivisionRecallIndex = String(nextIndex); } catch (_) {}
