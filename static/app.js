// ============================================
// Antigravity UI — Main Application Orchestrator
...
function _isForegroundChatBusy() {
  ...
  return !!window.__multivisionChatBusy
    || Date.now() < (window.__multivisionChatBusyUntil || 0)
...
function _submitMobileQueuedInput(input) {
  ...
  window.__multivisionQueueStreamingSubmit = now;
...
try {
  const cachedDefaultChat = JSON.parse(localStorage.getItem('antigravity-default-chat-cache') || 'null');
  if (cachedDefaultChat && cachedDefaultChat.endpoint_url && cachedDefaultChat.model) {
    _defaultChat = cachedDefaultChat;
    window.__multivisionDefaultChat = cachedDefaultChat;
  }
} catch (_) {}
async function _refreshDefaultChat() {
  try {
    ...
    if (d && d.endpoint_url && d.model) {
      _defaultChat = d;
      try {
        window.__multivisionDefaultChat = d;
        localStorage.setItem('antigravity-default-chat-cache', JSON.stringify(d));
      } catch (_) {}
      return d;
    }
  } catch (_) {}
...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  if (exportPdfBtn) {
    exportPdfBtn.addEventListener('click', (e) => {
      ...
      const sessionName = meta ? meta.name : 'Antigravity Chat';
  ...
  if (modelSortBtn && modelSortDropdown) {
    ...
    modelSortDropdown.querySelectorAll('.sort-option').forEach(opt => {
      opt.addEventListener('click', () => {
        const mode = opt.dataset.sort;
        Storage.set('antigravity-model-sort', mode);
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  function applyModeToToggles(mode) {
    ...
    window.__multivisionSetChatMode = setMode;
  ...
  (function initPlanToggle() {
    ...
    window.__multivisionSetPlanMode = (active) => setPlanMode(active, { silent: true });
    const statusToggle = el('plan-mode-status-toggle');
    if (btn) {
      btn.addEventListener('click', () => {
        const st = loadToggleState();
    ...
    if (msgInput && !msgInput._multivisionPlanTabToggle) {
      msgInput._multivisionPlanTabToggle = true;
    ...
    if (chatBar && !chatBar._multivisionPlanSwipeToggle) {
      chatBar._multivisionPlanSwipeToggle = true;
      let touchStartX = 0;
      let touchStartY = 0;
      let touchStartAt = 0;
  ...
  const SPLASH_COUNT_KEY = 'antigravity-tool-splash-counts';
  const SPLASH_MAX = 2;
  const _toolSplashes = {
  ...
  (function initModelPickerResponsive() {
    ...
    const TOOLBAR_HIDE_WIDTH = 160;
	    ...
	    function setComposerPlaceholder(width) {
	      ...
	      textarea.setAttribute('placeholder', width < PLACEHOLDER_COMPACT_WIDTH ? 'Message...' : 'Message Antigravity...');
	    }

	    if (_isMobile && textarea && !textarea._multivisionPlanPlaceholderHint) {
	      textarea._multivisionPlanPlaceholderHint = true;
	      setInterval(() => {
  ...
  if (incognitoBtn) {
    ...
    incognitoBtn.addEventListener('click', () => {
      ...
      if (chk.checked) {
        ...
        if (typeof window.__multivisionSetChatMode === 'function') {
          window.__multivisionSetChatMode('chat');
        } else {
        ...
        if (typeof window.__multivisionSetChatMode === 'function') {
          window.__multivisionSetChatMode(_restoreMode === 'chat' ? 'chat' : 'agent');
        }
  ...
  const UI_VIS_KEY = 'antigravity-ui-visibility';
  ...
  (function migrateOldToolbarVis() {
    const OLD_KEY = 'antigravity-toolbar-visibility';
    try {
  ...
  if (textarea) {
    ...
    window.addEventListener('antigravity:chat-busy-change', () => _syncMobileEnterKeyHint(textarea));
      ...
      if (_shouldQueueFromMobileEnter(e, textarea) || (e.key === 'Enter' && !e.shiftKey && !e.isComposing && !isMobile)) {
        ...
        if (form) {
          if (_isForegroundChatBusy() && textarea.value && textarea.value.trim()) {
            ...
            window.__multivisionQueueStreamingSubmit = Date.now();
          }
          _submitChatFormDirect(form);
        }
      }
...
// ============================================
function startMultivisionApp() {
  tasksModule?.startNotificationPolling?.();
  if (window.__multivisionAppStarted) return;
  window.__multivisionAppStarted = true;
  const _bumpChatPriority = (ms = 10000) => {
    try {
      window.__multivisionChatBusyUntil = Math.max(window.__multivisionChatBusyUntil || 0, Date.now() + ms);
    } catch (_) {}
  };
  ...
  if (documentModule) {
    ...
    if (_curSession && localStorage.getItem('antigravity-doc-open-' + _curSession) === '1') {
  ...
  window._multivisionBtnIcons = { send: _sendIcon, mic: _micIcon, stop: _stopIcon, newChat: _newChatIcon };
  ...
  if (sendBtn) {
    sendBtn.addEventListener('click', (e) => {
      ...
      if (sendBtn.dataset.mode === 'streaming') {
        if (hasText) window.__multivisionQueueStreamingSubmit = Date.now();
        handleSubmit(e);
        return;
      }
  ...
  const runNonCriticalStartup = (fn, delay = 4000) => {
    let tries = 0;
    const run = () => {
      const busy = !!window.__multivisionChatBusy
        || Date.now() < (window.__multivisionChatBusyUntil || 0)
        || !!document.querySelector('.send-btn[data-mode="streaming"], .send-btn.send-pending');
      if (busy && tries < 12) {
        tries += 1;
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', startMultivisionApp, { once: true });
} else {
  startMultivisionApp();
}
